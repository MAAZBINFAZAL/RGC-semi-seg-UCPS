...
def uncertainty_weight(logits):
    """
    Entropy-based confidence map for uncertainty-aware CPS.
...
class SSEGCPS:
    ...
    def __init__(self, args, hyp, weight_unsup=None):
        ...
        # Unsupervised loss
        # reduction='none' is needed because UCPS applies voxel-wise confidence weighting.
        self.loss_cps = nn.CrossEntropyLoss(reduction='none')
        ...
        if hyp['lr_scheduler'] == 'poly':
            ...
            self.lr_scheduler1 = CustomPolynomialLR(
                self.optimizer1,
                power=0.9,
                total_iters=self.epochs * math.ceil(self.epoch_samples_train / self.batch_size),
            ...
            self.lr_scheduler2 = CustomPolynomialLR(
                self.optimizer2,
                power=0.9,
                total_iters=self.epochs * math.ceil(self.epoch_samples_train / self.batch_size),
                warmup_epochs=warmup_epochs * math.ceil(self.epoch_samples_train / self.batch_size)
    ...
    def load_checkpoint(self) -> (int, dict):
        ...
        if not os.path.exists(ckpt_path):
            ...
            return 0, {
                'model_1': {'loss': math.inf, 'ap': 0, 'f1': 0, 'epoch': -1},
    ...
    def _is_better(self, model_scores, best_model_info):
        if self.metric == 'ap':
            return True if model_scores[1] > best_model_info['ap'] else False

        if self.metric == 'loss':
            return True if model_scores[0] < best_model_info['loss'] else False
    ...
    def run_epoch(self, loader, epoch):
        ...
        for curr_iter, batch_info in enumerate(progress_bar):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16, enabled=self.mixed_precision):
                ...
                if self.train_type == 'semi-supervised':
                    ...
                    pred_1 = torch.cat([logits_1, un_logits_1], dim=0)
                    pred_2 = torch.cat([logits_2, un_logits_2], dim=0)

                    # Pseudo labels from peer network
                    ...
                    # Entropy-based confidence maps
                    confidence_1 = uncertainty_weight(pred_1)
                    confidence_2 = uncertainty_weight(pred_2)

                    # Voxel-wise CPS loss
                    loss_map_1 = self.loss_cps(pred_1, max_2)
                    loss_map_2 = self.loss_cps(pred_2, max_1)

                    # Uncertainty-weighted Cross Pseudo Supervision
                    loss_cps_1 = (loss_map_1 * confidence_2).mean()
                    loss_cps_2 = (loss_map_2 * confidence_1).mean()

                    avg_conf_1 = confidence_1.mean().detach()
                    avg_conf_2 = confidence_2.mean().detach()
            ...
            if un_logits_1 is not None and (un_logits_1.isnan().any() or un_logits_1.isinf().any()):
                print(f'Nan/Inf values detected in the predicted unsupervised logits from model 1. epoch={epoch}')
                continue

            if un_logits_2 is not None and (un_logits_2.isnan().any() or un_logits_2.isinf().any()):
                print(f'Nan/Inf values detected in the predicted unsupervised logits from model 2. epoch={epoch}')
                continue

            weight_cps = self.weight_unsup(epoch=epoch, curr_iter=curr_iter) if self.weight_unsup else 0.0

            loss_total_1 = loss_ce_1 + weight_cps * loss_cps_1 if loss_cps_1 is not None else loss_ce_1
            loss_total_2 = loss_ce_2 + weight_cps * loss_cps_2 if loss_cps_2 is not None else loss_ce_2
            ...
            if avg_conf_1 is not None:
                loss_details['ucps_conf_1'].append(avg_conf_1.item())
                loss_details['ucps_conf_2'].append(avg_conf_2.item())
            ...
            progress_bar.set_description(
                ' '.join([
                    f'{key}: {np.mean(values):.8f}'
                    for key, values in loss_details.items()
                    if len(values) > 0
                ]) +
                f" GPU memory: {torch.cuda.max_memory_allocated() / 1024 ** 3:.2f} GB" +
                (f' lr: {self.lr_scheduler1.get_last_lr()[0]}' if self.lr_scheduler1 is not None else '')
            )
        ...
        if len(loss_details['loss_cps_1']) > 0:
        ...
        if len(loss_details['ucps_conf_1']) > 0:
            self.tfb_writer.add_scalar(f'UCPS_Confidence_1/train', np.mean(loss_details['ucps_conf_1']), epoch)
            self.tfb_writer.add_scalar(f'UCPS_Confidence_2/train', np.mean(loss_details['ucps_conf_2']), epoch)
    ...
    def train(self, train_loader, val_data_cfg=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            if val_data_cfg:
                scores_m1 = self.evaluator.evaluate(
                    self.model1,
                    self.loss_ce,
                    val_data_cfg,
                    'val',
                ...
                scores_m2 = self.evaluator.evaluate(
                    self.model2,
                    self.loss_ce,
                    val_data_cfg,
                    'val',
